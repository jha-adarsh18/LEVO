...
class VOBackend:
    ...
    def _associate_points_vectorized(self, kf):
        ...
        # Gather all reference descriptors and IDs once
        all_ref_descs = []
        all_ref_ids = []
        ...
        # Process descriptors in chunks to reduce memory overhead
        chunk_size = 128
        n_desc = len(kf.descriptors)
        
        # Pre-create map_points set for faster lookup
    ...
    def local_bundle_adjustment(self, K):
        ...
        # Convert to list once and filter
    ...
    def _reprojection_residuals_vectorized(self, params, kfs, point_ids, K):
        ...
        # Convert point_ids to set for O(1) lookup
        point_ids_set = set(point_ids)
        
        for kf in kfs:
            # Faster filtering using set lookup
        ...
        all_pts_3d = np.ascontiguousarray(all_pts_3d)
        all_pts_2d = np.ascontiguousarray(all_pts_2d)
        
        R_mats = all_poses[:, :3, :3]
        t_vecs = all_poses[:, :3, 3]
        
        # Optimized transformation using broadcasting
        pts_cam = np.einsum('nij,nj->ni', R_mats.transpose(0, 2, 1), all_pts_3d - t_vecs)
        ...
        # Project points
        pts_proj = (K @ pts_cam[valid].T).T
        pts_proj = pts_proj[:, :2] / pts_proj[:, 2:3]
        
        # Compute differences
        diff = pts_proj - all_pts_2d[valid]
        
        # Optimized residual assignment
    ...
    def _cull_outlier_points_vectorized(self, K):
        for kf in self.keyframes:
            ...
            pts_cam = np.dot(pts_3d, R_mat.T) - np.dot(t_vec, R_mat.T)
            valid = pts_cam[:, 2] > 0
            ...
            errors = np.linalg.norm(pts_proj - obs_2d, axis=1)
    ...
    def _pgo_residuals(self, params, kf_map):
        ...
        # Build poses dict with pre-allocated arrays
        n_poses = len(kf_map)
        poses = {}
        ...
        # Pre-allocate residual array for better performance
